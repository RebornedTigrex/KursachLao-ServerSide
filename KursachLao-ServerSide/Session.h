...
// UPDATED: Session с shared_ptr дл€ sender lifetime
...
private:
        ...
        http::async_read(socket_, buffer_, req_,
            [self = shared_from_this()](beast::error_code ec, std::size_t bytes) {  // NEW: дебаг байты
                if (!ec) {
                    ...
                    std::cout << "End of stream Ч closing session" << std::endl;
                    // Graceful close
    ...
    void on_read() {
        // FIXED: make_shared без {} Ч используем default cb в ctor
        ...
        // Ћ€мбда дл€ after_write Ч захват sp_sender (copy shared) + self (no dangling)
        auto after_write = [self = shared_from_this(), sp_sender](beast::error_code ec) {
            if (ec == http::error::end_of_stream) {  // NEW: Client closed Ч normal, no re-read
        ...
        // FIXED: Set cb ѕќ—Ћ≈ создани€ sp_sender, но ƒќ handleRequest
        sp_sender->after_write_cb_ = after_write;

        // “еперь handleRequest: sender живЄт via sp, ref ok
