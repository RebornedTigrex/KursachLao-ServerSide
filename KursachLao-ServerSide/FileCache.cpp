...
namespace {
    ...
    std::chrono::system_clock::time_point file_time_to_system_time(
        const fs::file_time_type& ftime) {

        // Для std::filesystem используем duration_cast
...
// Реализация методов класса FileCache
...
// Вытеснение файлов при переполнении кэша
...
// Получение файла по прямому пути
std::optional<FileCache::CachedFile> FileCache::get_file_by_path(const std::string& file_path) {
    ...
    std::string temp_route = "/file" + std::to_string(
...
// Принудительное кэширование файла
...
// Получение MIME типа для маршрута
